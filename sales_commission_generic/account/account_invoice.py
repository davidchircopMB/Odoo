...
class WizardInvoiceSaleCommission(models.Model):
	...
	def print_commission_report(self):
		...
		datas = {
			...
			'user_id':self.salesperson.name
		}
		return self.env.ref('sales_commission_generic.report_pdf').report_action(self,data=datas)


class WizardInvoiceDesignerRoyalties(models.Model):
	_name = 'wizard.invoice.designer.royalties'
	_description = 'Wizard Invoice Designer Royalties'

	start_date = fields.Date('Start Date', required=True)
	end_date = fields.Date('End Date', required=True)
	partner_des_id = fields.Many2one('res.partner','Designer', required=True)
	
	def print_royalties_report(self):
		temp = []
		sale_invoice_commission_ids = self.env['invoice.sale.commission'].search([('date','>=',self.start_date),('date','<=',self.end_date),('partner_des_id','=',self.partner_des_id.id)])
		if not sale_invoice_commission_ids:
			raise UserError('There Is No Any Commissions.')
		else:
			for a in sale_invoice_commission_ids:
		...
		datas = {
			...
			'start_date':self.start_date,
			'end_date':self.end_date,
			'partner_des_id':self.partner_des_id.name
		}
		return self.env.ref('sales_commission_generic.report_des_pdf').report_action(self,data=datas)
...
class InvoiceSaleCommission(models.Model):
	...
	type_name = fields.Char(string="Royalty Name")
	comm_type = fields.Selection([
		('standard', 'Standard'),
		('partner', 'Partner Based'),
		('mix', 'Product/Category/Margin Based'),
		('discount', 'Discount Based'),
		], 'Royalty Type', copy=False, help="Select the type of commission you want to apply.")
	user_id = fields.Many2one('res.users', string='Sales Person',
								 help="sales person associated with this type of commission",
								 required=True)
	commission_amount = fields.Float(string="Royalty Amount")
	invoice_id = fields.Many2one('account.move', string='Invoice Reference',
								 help="Affected Invoice")
	invoice_line_id = fields.Many2one('account.move.line', string='line Reference',
								 help="Affected Invoice Line")
	order_id = fields.Many2one('sale.order', string='Order Reference',
								 help="Affected Sale Order")
	order_line_id = fields.Many2one('sale.order.line', string='line Reference',
								 help="Affected Sale Order line")
	commission_id = fields.Many2one('sale.commission', string='Designer Royalties',
								 help="Related Commission",)
	product_id = fields.Many2one('product.product', string='Product',
								 help="product",)
	partner_id = fields.Many2one('res.partner', string='Partner')
	partner_des_id = fields.Many2one('res.partner', string='Partner')
...
class AccountInvoice(models.Model):
	...
	partner_des_id = fields.Many2one('res.partner', string='Designer')
	...
	def get_standard_commission(self, commission_brw, invoice):
		...
		if invoice.move_type == 'out_invoice':
			for line in invoice.invoice_line_ids:
				if len(line._get_downpayment_lines()) == 0 and line.display_type != 'line_section' and line.display_type !='line_note':
					...
					if standard_invoice_commission_data and standard_commission_amount > 0:
						commission_line = invoice_commission_obj.search([('invoice_id', '=', invoice.id),('invoice_line_id', '=', line.id)])
			...
			for line in invoice.invoice_line_ids:
				if len(line._get_downpayment_lines()) == 0 and line.display_type != 'line_section' and line.display_type !='line_note':
					...
					if (invoice.user_id and invoice.user_id.id in sales_person_list) and invoice.partner_id.is_affiliated == True:
						...
						name = 'Partner Based commission " '+ tools.ustr(commission_brw.name) +' (' + tools.ustr(commission_brw.affiliated_partner_commission) + '%)" for Affiliated Partner "' + tools.ustr(invoice.partner_id.name) + '"'
						...
						name = 'Partner Based commission " '+ tools.ustr(commission_brw.name) +' (' + tools.ustr(commission_brw.nonaffiliated_partner_commission) + '%)" for Non-Affiliated Partner "' + tools.ustr(invoice.partner_id.name) + '"'
						invoice_commission_data = {'name' : name,
										   'user_id' : invoice.user_id.id,
										   'partner_id' : invoice.partner_id.id,
										   'commission_id' : commission_brw.id,
				...
				if len(line._get_downpayment_lines()) == 0 and line.display_type != 'line_section' and line.display_type !='line_note':
					...
					for exception in exception_ids:
						...
						if exception.based_on_2 == 'Fix Price':
							amount = line.price_subtotal
						elif exception.based_on_2 == 'Royalty Exception':
							amount = line.price_subtotal
						else:
							amount = margin * line.quantity
						margin_percentage = exception.margin_percentage

						if len(exception.product_id.designer_ids) > 0:
							for des in exception.product_id.designer_ids:
								...
								if exception.based_on == 'Products':
									...
									name = 'Royalty Exception for ' + tools.ustr(exception.based_on) + ' "' + tools.ustr(exception.product_id.name) + '" @' + tools.ustr(commission_precentage) + '%'
								elif exception.based_on == 'Product Categories':
									categ_id = exception.categ_id.id
									name = 'Royalty Exception for ' + tools.ustr(exception.based_on) + ' "' + tools.ustr(exception.categ_id.name) + '" @' + tools.ustr(commission_precentage) + '%'
								elif exception.based_on == 'Product Sub-Categories':
									sub_categ_id = exception.sub_categ_id.id
									name = 'Royalty Exception for ' + tools.ustr(exception.based_on) + ' "' + tools.ustr(exception.sub_categ_id.name) + '" @' + tools.ustr(commission_precentage) + '%'
								invoice_commission_data = {'name': name,
														   ...
														   'user_id' : invoice.user_id.id,
														   'partner_des_id' : des.partner_des_id.id,
			...
			if move.is_invoice(include_receipts=True) and move.state == 'posted':
				if currency.is_zero(move.amount_residual):
					if all(payment.is_matched for payment in move._get_reconciled_payments()):
						...
						commission_configuration = self.env.company.commission_configuration
...
class AccountMoveReversal(models.TransientModel):
	...
	def reverse_moves(self):
		...
		if active_id:
			...
			if move.commission_ids :
				move.commission_ids.sudo().unlink()
		return res
