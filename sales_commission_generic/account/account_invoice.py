...
class WizardInvoiceDesignerRoyalties(models.Model):
	...
	def print_royalties_report(self):
		temp = []
		sale_invoice_commission_ids = self.env['invoice.sale.commission'].search([('date','>=',self.start_date),('date','<=',self.end_date),('partner_des_id','=',self.partner_des_id.id),('state','!=','cancel')])
		if not sale_invoice_commission_ids:
			raise UserError('There Is No Any Commissions.')
		else:
			for a in sale_invoice_commission_ids:
...
class InvoiceSaleCommission(models.Model):
	...
	currency_id = fields.Many2one('res.currency', string='Invoice Currency',tracking=True)
