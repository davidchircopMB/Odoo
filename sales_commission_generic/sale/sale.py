...
class SaleOrder(models.Model):
	...
	def get_mix_commission(self, commission_brw, order):
		...
		for line in order.order_line:
			...
			#================================================================================
			cost = line.product_id.list_price
			if cost == 0.0:
...
class SaleOrderLine(models.Model):
	...
	def _compute_qty_delivered(self):
		...
		for line in self:
			if line.order_id.delivery_count > 0:
				move_done = self.env['stock.move'].search([('sale_line_id','=',line.id),('state','=','done')])
				if len(move_done) > 0:
					if line.qty_delivered == 0:
						commission_line = self.env['invoice.sale.commission'].search([('order_line_id','=',line.id)])
						if commission_line:
							if commission_line.state == 'to_invoice':
